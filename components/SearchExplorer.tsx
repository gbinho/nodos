...
export function SearchExplorer({ initialUsers, initialFollowing, currentUserId }: { initialUsers: SearchUser[]; initialFollowing: string[]; currentUserId: string }) {
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-8">
      <header><p className="text-xs font-medium uppercase tracking-[0.18em] text-[#71737c]">EXPLORAR</p><h1 className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-[#111114]">Buscar na rede</h1><p className="mt-2 text-sm text-[#71737c]">Encontre pessoas e interesses para acompanhar.</p></header>
      <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8b8d96]" strokeWidth={1.5} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar username ou hobby..." className="w-full rounded-xl border border-[#e4e5e9] bg-white py-3 pl-10 pr-4 text-sm text-[#111114] outline-none placeholder:text-[#a0a1a8] focus:border-[#111114]" /></div>
      <div className="flex gap-1 border-b border-[#e4e5e9]"><button type="button" onClick={() => setTab("users")} className={`border-b-2 px-4 py-3 text-sm ${tab === "users" ? "border-[#111114] text-[#111114]" : "border-transparent text-[#8b8d96] hover:text-[#111114]"}`}><UsersRound className="mr-2 inline h-4 w-4" strokeWidth={1.5} />Usuários</button><button type="button" onClick={() => setTab("tags")} className={`border-b-2 px-4 py-3 text-sm ${tab === "tags" ? "border-[#111114] text-[#111114]" : "border-transparent text-[#8b8d96] hover:text-[#111114]"}`}><Search className="mr-2 inline h-4 w-4" strokeWidth={1.5} />Tags/Hobbies</button></div>
