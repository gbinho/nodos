    ...
    <article className={`feed-card overflow-hidden rounded-2xl border bg-white shadow-[0_8px_30px_rgba(22,22,28,0.06)] ${featured ? "border-[#111114]" : "border-[#e4e5e9]"}`}>
      ...
      <div className={gallery ? "p-4" : "p-5"}>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-[#ececf0] pt-4">
          ...
          {reactionOptions.map(({ type, label, emoji }) => <button key={type} type="button" onClick={() => void toggleReaction(type)} disabled={loading || reactionLoading !== null} aria-pressed={myReaction === type} className={`rounded-lg border px-2.5 py-1.5 text-xs transition-all duration-200 hover:-translate-y-0.5 active:scale-95 ${myReaction === type ? "border-[#111114] bg-[#111114] text-white" : "border-[#d7d8dd] text-[#111114] hover:border-[#111114]"}`}>{reactionLoading === type ? "..." : `${emoji} ${label} ${counts[type]}`}</button>)}
          <button type="button" onClick={() => setCommentsOpen((open) => !open)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#d7d8dd] px-2.5 py-1.5 text-xs text-[#111114] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#111114] active:scale-95"><MessageCircle className="h-3.5 w-3.5" strokeWidth={1.7} />Comentários ({comments.length})</button>
          <button type="button" onClick={() => setShareOpen(true)} className="inline-flex items-center gap-1.5 rounded-lg border border-[#d7d8dd] px-2.5 py-1.5 text-xs text-[#111114] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#111114] active:scale-95"><Send className="h-3.5 w-3.5" strokeWidth={1.7} />Compartilhar</button>
        </div>
