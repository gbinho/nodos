    ...
    <main className="mx-auto flex max-w-3xl flex-col gap-8">
      ...
      <header className="border border-gray-800 bg-gray-950 p-6">
        <div className="flex items-center gap-5">
          ...
          <div><h1 className="text-2xl font-medium">@{publicProfile.username ?? "usuário"}</h1><p className="mt-2 flex items-center gap-2 text-sm text-gray-500"><Mail className="h-3.5 w-3.5" strokeWidth={1.5} />{publicProfile.email_public ? publicProfile.email ?? "E-mail não informado" : "E-mail oculto"}</p><p className="mt-2 text-sm text-gray-400">{publicProfile.total_xp} XP</p></div>
        </div>
      </header>
